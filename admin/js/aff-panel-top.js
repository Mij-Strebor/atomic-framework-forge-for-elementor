  ...
  AFF.PanelTop = {
      ...
      AFF.App.ajax("aff_get_diagnostics", { filename: AFF.state.currentFile })
        .then(function (res) {
          ...
          if (!hasProblems) {
            ...
            if (dupVars.length > 0) {
              body +=
                '<p style="color:#e07a40;font-weight:600;margin-bottom:6px">' +
                dupVars.length +
    ...
    _openManageProject: function () {
      ...
      function _catsToStr(arr) {
        return (arr || [])
          .filter(function (c) {
            return !c.locked && c.name !== "Uncategorized";
          })
          .map(function (c) {
            return c.name;
          })
      ...
      var colorsStr =
        _catsToStr(cfg.colorCategories || cfg.categories) ||
      ...
      requestAnimationFrame(
        function () {
          ...
          AFF.App.ajax("aff_get_settings", {}).then(function (res) {
            ...
            if (selFonts && s.fonts_default_type) {
              selFonts.value = s.fonts_default_type;
            }
    ...
    _saveProjectConfig: function () {
      ...
      function _buildCatArray(newNames, existingArr) {
        ...
        for (var i = 0; i < existingArr.length; i++) {
          byName[existingArr[i].name] = existingArr[i];
        }
        var result = [];
      ...
      var newColCats = _buildCatArray(
        colNewNames,
        cfg.colorCategories || cfg.categories || [],
      );
      ...
      function _uncatIdFor(catArr) {
        for (var i = 0; i < catArr.length; i++) {
          if (catArr[i].name === "Uncategorized") {
            return catArr[i].id;
          }
        }
      ...
      for (var vi = 0; vi < vars.length; vi++) {
        ...
        if (v.category_id) {
          var newCatId = null;
          if (v.subgroup === "Colors" && removedColIds[v.category_id]) {
            newCatId = colUncatId;
          }
          if (v.subgroup === "Fonts" && removedFntIds[v.category_id]) {
            newCatId = fntUncatId;
          }
          if (v.subgroup === "Numbers" && removedNumIds[v.category_id]) {
            newCatId = numUncatId;
          }
          if (newCatId) {
        ...
        .then(function (res) {
          if (res && res.success) {
            ...
            AFF.Modal.open({
              ...
              body:
                "<p>" +
                ((res && res.data && res.data.message) || "Unknown error.") +
                "</p>",
            });
          }
        })
    ...
    _postSyncRefresh: function (source, addedCount, resolvedCount) {
      ...
      if (AFF.Variables && AFF.Variables._sets) {
        ...
        if (_vsets["Numbers"] && _vsets["Numbers"]._ensureUncategorized) {
      ...
      if (addedCount > 0) {
        parts.push(
          addedCount + " variable" + (addedCount !== 1 ? "s" : "") + " added",
        );
      }
      if (resolvedCount > 0) {
        parts.push(
          ...
          if (importBtn) {
            importBtn.addEventListener("click", function () {
              ...
              reader.onload = function (e) {
                self._applyImport(e.target.result, statusEl);
              };
    ...
    _applyImport: function (text, statusEl) {
      ...
      var importedName = (parsed.name || "").replace(
        /(?:\.eff)+(?:\.json)?$/i,
        "",
      );
