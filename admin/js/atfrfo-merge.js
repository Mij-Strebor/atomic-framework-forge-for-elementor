/**
 * ATFRFO Merge — Conflict Detection and Resolution
 *
 ...
 *
 * Public API:
 *   ATFRFO.Merge.buildConflictList(elementorVars, affVars)
 *     → { newVars, conflictVars, matchVars }
 *
 *   ATFRFO.Merge.openMergeDialog(conflictVars, direction, onApply, onCancel)
 *     direction: 'fetch' | 'write'
 *     onApply  : called with [{name, winner:'atfrfo'|'el'}, ...]
...
(function () {
	...
	ATFRFO.Merge = {
		...
		/**
		 * Partition Elementor variables against the current ATFRFO variable set.
		 *
		 * @param {Array} elementorVars  [{name, value}, ...] — from atfrfo_sync_from_elementor
		 * @param {Array} affVars        ATFRFO.state.variables
		 * @returns {{ newVars: Array, conflictVars: Array, matchVars: Array }}
		 *   newVars      — name not in ATFRFO (should be added as normal)
		...
		buildConflictList: function (elementorVars, affVars) {
			...
			// Build a lowercase-name → current value map from ATFRFO state.
			// We preserve the original ATFRFO value for display but compare lower-cased.
			var affMap = {};
			...
			elementorVars.forEach(function (v) {
				...
				if (!(nameLc in affMap)) {
					...
					if (affNorm === elNorm) {
						...
						conflictVars.push({
							name:     nameLc,
							affValue: affMap[nameLc], // original ATFRFO value (display)
		 ...
		 * @param {Function} onApply       called with [{name, winner:'atfrfo'|'el'}, ...]
		...
		openMergeDialog: function (conflictVars, direction, onApply, onCancel) {
			...
			conflictVars.forEach(function (c) {
				var radioName = 'atfrfo-merge-' + c.name.replace(/[^a-z0-9]/g, '-');
				rowsHtml +=
					'<tr data-var-name="' + ATFRFO.Utils.escHtml(c.name) + '"'
					+ ' style="border-bottom:1px solid var(--atfrfo-clr-border)">'
					+ '<td style="padding:6px 8px 6px 0;font-size:12px;color:var(--atfrfo-clr-primary)'
					+   ';white-space:nowrap;min-width:160px">'
					+   '<code style="font-size:11px">' + ATFRFO.Utils.escHtml(c.name) + '</code>'
					+ '</td>'
					+ '<td style="padding:6px 8px;font-size:12px;color:var(--atfrfo-clr-secondary)'
					+   ';white-space:nowrap">'
					+   self._swatch(c.atfrfoValue) + ATFRFO.Utils.escHtml(c.atfrfoValue)
					+ '</td>'
					+ '<td style="padding:6px 8px;font-size:12px;color:var(--atfrfo-clr-secondary)'
					+   ';white-space:nowrap">'
					+   self._swatch(c.elValue) + ATFRFO.Utils.escHtml(c.elValue)
					+ '</td>'
			...
			var body   =
				...
				+ '<thead>'
				+ '<tr style="border-bottom:2px solid var(--atfrfo-clr-border)">'
				...
				+   ';color:var(--atfrfo-clr-muted);font-weight:600">Keep</th>'
				+ '</tr>'
				+ '</thead>'
			...
			var footer =
				...
				+ '<button class="atfrfo-btn atfrfo-btn--secondary"'
				+   ' id="atfrfo-merge-cancel">Cancel</button>'
				+ '<button class="atfrfo-btn" id="atfrfo-merge-apply">Apply &amp; Continue</button>'
				+ '</div>';
			...
			mergeHandler = function (e) {
				if (e.target.id === 'atfrfo-merge-cancel') {
					document.removeEventListener('click', mergeHandler);
					ATFRFO.Modal.close();
					if (onCancel) { onCancel(); }

				} else if (e.target.id === 'atfrfo-merge-keep-all-aff') {
					self._setAll('atfrfo', conflictVars);

				} else if (e.target.id === 'atfrfo-merge-use-all-el') {
					self._setAll('el', conflictVars);

				} else if (e.target.id === 'atfrfo-merge-apply') {
					document.removeEventListener('click', mergeHandler);
		...
		_swatch: function (value) {
			...
			if (!ATFRFO.Utils || !ATFRFO.Utils.isColorValue(lc)) { return ''; }
			return '<span style="display:inline-block;width:12px;height:12px'
				+ ';border-radius:2px;border:1px solid var(--atfrfo-clr-border)'
				+ ';background:' + ATFRFO.Utils.escHtml(value)
		 ...
		 * Falls back to 'atfrfo' if a radio group cannot be found.
		 *
		 * @param {Array} conflictVars
		 * @returns {Array}  [{name, winner: 'atfrfo'|'el'}, ...]
		 */
		_collectResolutions: function (conflictVars) {
			return conflictVars.map(function (c) {
				...
				return { name: c.name, winner: checked ? checked.value : 'atfrfo' };
			});
		},
