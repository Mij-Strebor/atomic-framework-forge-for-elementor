...
ATFRFO uses a single shared tooltip system for all interactive elements across the entire admin interface — top bar buttons, left panel, right panel, category headers, variable rows, and any dynamically created controls. Every tooltip in the application is rendered through one `#atfrfo-tooltip` DOM element and one set of delegated event listeners.
...
Declared once in `admin/views/page-atfrfo-main.php`, outside the main scroll container, as a direct child of `.atfrfo-app`:
...
|---|---|---|
| `class` | `atfrfo-tooltip` | CSS styling hook |
| `id` | `atfrfo-tooltip` | JS reference (`document.getElementById`) |
| `role` | `tooltip` | ARIA landmark |
| `aria-hidden` | `true` (default) / `false` (when visible) | Accessibility state |

### 2.2 CSS — `admin/css/atfrfo-theme.css`

```css
.atfrfo-app .atfrfo-tooltip {
    position:       fixed;          /* viewport-relative; scrollY is NOT added in JS */
    z-index:        99999;
    background:     var(--atfrfo-clr-primary);     /* deep brown */
    color:          #faf6f0;                    /* light cream */
    font-size:      var(--fs-xs);               /* 11px */
    font-weight:    var(--fw-medium);           /* 500 */
    padding:        var(--sp-1) var(--sp-2);    /* 4px 8px */
    border-radius:  var(--atfrfo-border-radius-sm); /* 4px */
    ...
    box-shadow:     var(--atfrfo-shadow-sm);
}

.atfrfo-app .atfrfo-tooltip.is-visible {
    opacity:   1;
...
| Attribute | Purpose |
|---|---|
| `data-atfrfo-tooltip="Short label"` | Text shown in standard tooltip mode |
| `data-atfrfo-tooltip-long="Full sentence."` | Text shown in Extended tooltip mode (see §6) |

If an element has `data-atfrfo-tooltip-long` but not `data-atfrfo-tooltip`, it will not trigger a tooltip in standard mode.

If an element has `data-atfrfo-tooltip` but not `data-atfrfo-tooltip-long`, both standard and extended mode show the same short text.

---

## 4. Event Binding — `ATFRFO.PanelTop._bindTooltips()`
...
_bindTooltips: function () {
    ...
    document.addEventListener('mouseover', function (e) {
        var target = e.target.closest('[data-atfrfo-tooltip]');
        if (target !== _tipEl) {
    ...
    document.addEventListener('focusin', function (e) {
        if (e.target && e.target.getAttribute && e.target.getAttribute('data-atfrfo-tooltip')) {
...
**No other module binds tooltip events.** Any previous per-container or per-element tooltip binding in feature modules (`atfrfo-colors.js`, etc.) is disabled. The global delegated system handles everything.

---

## 5. Show Behaviour — `ATFRFO.PanelTop._showTooltip(anchor)`
...
_showTooltip: function (anchor) {
    ...
    var text = this._extendedTooltips
        ? (anchor.getAttribute('data-atfrfo-tooltip-long') || anchor.getAttribute('data-atfrfo-tooltip'))
...
## 6. Hide Behaviour — `ATFRFO.PanelTop._hideTooltip()`
...
Two settings are persisted via `atfrfo_save_settings` / `atfrfo_get_settings` (WordPress options):

| Setting key | Type | Default | Description |
|---|---|---|---|
| `show_tooltips` | boolean | `true` | When `false`, no tooltips appear anywhere in the UI |
| `extended_tooltips` | boolean | `false` | When `true`, `data-atfrfo-tooltip-long` text is preferred over `data-atfrfo-tooltip` |

### 7.2 Runtime Properties

`ATFRFO.PanelTop` exposes two properties that reflect the persisted settings:

```javascript
ATFRFO.PanelTop._showTooltips     // boolean, default true
ATFRFO.PanelTop._extendedTooltips // boolean, default false
```

These are loaded from settings on `init()` via an async `atfrfo_get_settings` AJAX call. They update silently (non-blocking) after DOM ready.
...
1. Updates the corresponding `ATFRFO.PanelTop._showTooltips` / `ATFRFO.PanelTop._extendedTooltips` property
2. Calls `atfrfo_save_settings` to persist the value
...
<button data-atfrfo-tooltip="Save file">…</button>

<!-- Full: short + extended -->
<button
  data-atfrfo-tooltip="Save"
  data-atfrfo-tooltip-long="Save the current project to the selected .atfrfo.json file">
  …
</button>
```

No JavaScript binding is needed. The delegated `document` listener picks up any element with `data-atfrfo-tooltip` automatically — including elements appended to the DOM after page load.

---

## 9. Elements That Must Have Tooltips

### 9.1 Top Bar (static, `page-atfrfo-main.php`)

All icon-only buttons in the top bar carry both `data-atfrfo-tooltip` and `data-atfrfo-tooltip-long`.
...
| Export | `Export` | `Export the current project as a .atfrfo.json file` |
| Import | `Import` | `Import a .atfrfo.json file into the current project` |
...
### 9.2 Variable Rows (dynamic, `atfrfo-colors.js`)

All action buttons on variable rows carry `data-atfrfo-tooltip` (short) and where appropriate `data-atfrfo-tooltip-long` (extended). Category-level action buttons use `data-atfrfo-tooltip` via the `_catBtn()` helper.
...
The status dot uses `_statusLongTooltip(status)` for its `data-atfrfo-tooltip-long` value:
...
| `orphaned` | `Orphaned` | `Orphaned — Exists in ATFRFO but not found in Elementor kit. Commit to add it.` |

---

## 10. What Tooltips Are Not

- **Not a separate system per module.** Feature modules (`atfrfo-colors.js`, etc.) do not bind their own tooltip events. Only `ATFRFO.PanelTop._bindTooltips()` does.
...
- **Not an error display.** Inline field validation errors use a separate `.atfrfo-inline-error` element with its own styling and are not part of this system.
...
- [ ] Add `data-atfrfo-tooltip="short text"` to the element
- [ ] Add `data-atfrfo-tooltip-long="Full sentence."` if the element benefits from extended description
