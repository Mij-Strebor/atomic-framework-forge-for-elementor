# ATFRFO Phase 3 — Classes Support: Implementation Plan

**Document version:** 1.0  
**Date:** 2026-04-03  
**Author:** Research + planning document for Jim Roberts / JimRForge  
**ATFRFO version target:** v0.4.0-beta  
...
6. [PHP API Approach — ATFRFO_Classes_Reader](#6-php-api-approach--atfrfo_classes_reader)
7. [ATFRFO Class Data Model — JSON Schema](#7-atfrfo-class-data-model--json-schema)
...
11. [ATFRFO_Data_Store Extensions](#11-atfrfo_data_store-extensions)
...
- **Not present** in the kit CSS file (`post-{id}.css`) that ATFRFO's current CSS parser reads
...
This has a material impact on ATFRFO Phase 3. The ATFRFO CLAUDE.md note — *"Elementor v4 not yet exposing these"* — was accurate at time of writing and remains partially true: Elementor now stores and exposes class data via its own internal REST API, but there is no documented, stable PHP hook or filter for third-party plugins to read Global Class definitions programmatically.

**Phase 3 is implementable**, but ATFRFO must use a direct database/REST read strategy rather than a clean PHP API. The plan below describes the safest approach: reading Elementor's class data from `wp_postmeta` on the active kit post, with a REST-based fallback.
...
2. One or more optional **Global Classes** — user-defined, reusable, displayed in green. Managed via the Class Manager. The subject of ATFRFO Phase 3.
...
In earlier Alpha builds, Elementor auto-generated opaque identifiers as class names in the DOM. This was fixed in 3.30: **all Global Classes now render with their user-defined names in the HTML DOM**. A class named `primary-btn` appears as `class="primary-btn"` on the element. This matters for ATFRFO's display: the class name stored in the database is the same class name in the markup.
...
Elementor stores its kit-level data (global colors, typography, settings, and in V4 global classes) as post meta on the **active kit post**. The active kit ID is available from `get_option('elementor_active_kit')`, which ATFRFO already uses in `ATFRFO_CSS_Parser::get_active_kit_id()`.
...
#### Mapping to ATFRFO data model

| Elementor field | ATFRFO field | Notes |
|-----------------|-----------|-------|
| `id` (e.g. `g-19ae5e7`) | `elementor_id` | Stored verbatim — not converted |
| `label` | `name` AND `label` | Serves both roles; ATFRFO `label` starts as a copy and can be overridden |
| `type` | (discarded) | Always "class"; ATFRFO infers from context |
| `variants` | (phase 3.4+) | Style data — not parsed in Phase 3.1/3.2 |
| `meta.order` index | `order` | Used to set initial sort order |

**ATFRFO will use the GET endpoint as the primary read source.** This is safer than raw `wp_postmeta` queries because:
...
**Authentication:** The REST endpoint uses WordPress nonce authentication. ATFRFO will use `wp_create_nonce('wp_rest')` on the server side and pass it in `X-WP-Nonce` headers from JS, or call it server-side from a PHP AJAX handler using `wp_remote_get()`.

### 3.3 Export/Import JSON (Supplementary Source)

When a user exports Classes and Variables from Elementor (via Elementor > Tools > Export), they receive a JSON file. ATFRFO could parse an exported file as a manual import path if the REST approach fails. This is a fallback, not the primary path.
...
| 3 (manual) | User-uploaded Elementor export JSON | File parse in ATFRFO | For offline/exported sites |

---

## 4. CSS Output — What Goes in What File

This section is critical because it defines the boundary between ATFRFO's existing CSS parser and the new classes reader.

### 4.1 Kit CSS File (`post-{id}.css`) — Variables Only

The kit CSS file that ATFRFO currently parses contains:
- **Block 1:** Legacy `--e-global-*` variables (V3 global colors, typography as CSS custom properties)
- **Block 2 (terminal `:root`):** V4 user-defined atomic variables — the ones ATFRFO reads
...
**ATFRFO Phase 3 does not need to parse this CSS output.** ATFRFO reads the class definitions (names, metadata, applied elements) from the REST API/database, not from the generated stylesheet. The stylesheet is read-only output from Elementor; ATFRFO manages the definition layer.

---

## 5. Two Distinct Class Types

Understanding the distinction is important for ATFRFO's data model and UI.
...
- Not relevant to ATFRFO Phase 3 (these are per-page, per-element; too granular)
- **ATFRFO Phase 3 scope: EXCLUDE Local Classes**
...
- **ATFRFO Phase 3 scope: Global Classes only**

---

## 6. PHP API Approach — `ATFRFO_Classes_Reader`

Create a new file: `E:/projects/plugins/eff/includes/class-atfrfo-classes-reader.php`

This is a **read-only** class (parallel to `ATFRFO_CSS_Parser`, which is also read-only). It does not extend `ATFRFO_CSS_Parser` — it is a sibling class for a different data source.
...
class ATFRFO_Classes_Reader {
     ...
     *
     * @return array[] Array of ATFRFO class objects (see data model section).
     */
    ...
    public function normalize( array $raw ): array {
        ...
        foreach ( $data as $elementor_id => $entry ) {
            ...
            $classes[] = [
                'id'             => '',        // ATFRFO UUID assigned by ATFRFO_Data_Store::add_class()
                'elementor_id'   => $elementor_id,
                'name'           => $label,    // CSS class name — same as label in Elementor's model
                'label'          => $label,    // ATFRFO display label; user may override
                'type'           => 'global',
...
**Important:** `wp_remote_get()` makes an HTTP request from the server to itself. On some local environments (Local by Flywheel, WAMP) this can fail if the loopback URL is not reachable. ATFRFO must handle this gracefully and fall through to the `read_from_postmeta()` method.
...
public function read_from_postmeta(): array {
    ...
    $meta_key = apply_filters( 'atfrfo_global_classes_meta_key', '_elementor_global_classes' );
...
### 6.4 Critical Rules for `ATFRFO_Classes_Reader`

- **Read-only.** It never writes to Elementor's data. Same rule as `ATFRFO_CSS_Parser`.
...
- Prefix: `ATFRFO_Classes_Reader` — follows `ATFRFO_` naming rule.

---

## 7. ATFRFO Class Data Model — JSON Schema

### 7.1 The ATFRFO Class Object

Global Classes are stored in the `classes` array of the `.atfrfo.json` project file. Each class object:
...
|-------|------|-------------|
| `id` | string (UUID v4) | ATFRFO-generated internal identifier |
...
| `notes` | string | Developer notes (ATFRFO-only, not synced to Elementor) |
| `tags` | string[] | User-defined tags for filtering (ATFRFO-only) |
| `created_at` | ISO 8601 | When ATFRFO first recorded this class |
| `updated_at` | ISO 8601 | Last ATFRFO modification |
...
| Status | Meaning |
|--------|---------|
| `synced` | Class exists in Elementor and in ATFRFO file, data matches last sync |
| `modified` | Class has ATFRFO-side edits (notes, tags, category) not yet auto-persisted |
| `atfrfo-only` | Class exists in ATFRFO file but not found in Elementor (deleted from editor?) |
| `orphaned` | Class exists in Elementor but not in ATFRFO file (appeared since last sync) |

### 7.4 `.atfrfo.json` Storage Structure
...
The `config.classCategories` array follows the same pattern as `config.categories` (used for variable color categories). It uses ATFRFO_Data_Store's existing `subgroup_to_cat_key()` pattern — a new entry `'Classes' => 'classCategories'` is added to the map.
...
| `value` | *(no direct equivalent)* | Class styles live in Elementor; ATFRFO doesn't store individual CSS properties |
...
File: `admin/js/atfrfo-panel-left.js`
...
- Highlight active category with `--atfrfo-clr-accent`

No jQuery. Follow the existing vanilla JS patterns in `atfrfo-panel-left.js`.
...
- **Status indicator** — icon + color-coded: synced (green), modified (gold), atfrfo-only (muted), orphaned (red)
- **Usage count** — from last Elementor fetch
- **Actions menu [⋮]** — Edit notes, Move to category, Remove from ATFRFO

### 9.2 No Inline Editing of CSS Properties

ATFRFO Phase 3 **does not** allow editing the CSS properties of a class (colors, typography, spacing). That is Elementor's job, done in the Elementor editor. ATFRFO is a **management and documentation layer**, not a style editor for classes. This mirrors ATFRFO's approach to variables: ATFRFO organizes and annotates, Elementor owns the authoritative values.

What ATFRFO CAN edit on a class:
- `label` — human-readable display name (ATFRFO-only metadata)
- `category` — which ATFRFO category this class belongs to
- `notes` — developer notes
- `tags` — for filtering
- `order` — drag-to-reorder within a category

### 9.3 Detail/Expand Modal (`.atfrfo-class-detail-modal`)

Clicking a class opens a detail view in the existing ATFRFO modal system (`ATFRFO.Modal.open()`):
...
The "Edit in Elementor" link opens the Elementor editor (deep link to the kit/Class Manager if possible; otherwise just to the editor). This is a link only — ATFRFO does not open Elementor programmatically.

### 9.4 Sync Button

In the top bar or right panel, a "Sync Classes" button triggers `atfrfo_sync_classes` AJAX action. This:
1. Calls `ATFRFO_Classes_Reader::fetch_from_rest()` (or postmeta fallback)
2. Merges new/updated classes with the existing ATFRFO store
3. Marks disappeared classes as `orphaned`
4. Returns updated counts and status to the UI

The sync is **non-destructive**: ATFRFO-only metadata (labels, notes, tags, categories) is preserved.
...
All endpoints follow ATFRFO conventions: `wp_ajax_{action}`, nonce via `check_ajax_referer()`, `manage_options` capability check, response via `wp_send_json_success()` / `wp_send_json_error()`.

| Action | Handler Method | Description |
|--------|---------------|-------------|
| `atfrfo_sync_classes` | `ajax_aff_sync_classes()` | Fetch from Elementor, merge into store, return updated class list |
| `atfrfo_get_classes` | `ajax_aff_get_classes()` | Return stored classes (no Elementor fetch) |
| `atfrfo_update_class_meta` | `ajax_aff_update_class_meta()` | Update ATFRFO-only fields: label, notes, tags, category, order |
| `atfrfo_move_class_category` | `ajax_aff_move_class_category()` | Move class to a different category |
| `atfrfo_get_class_categories` | `ajax_aff_get_class_categories()` | Return class category list |
| `atfrfo_add_class_category` | `ajax_aff_add_class_category()` | Add a new class category |
| `atfrfo_rename_class_category` | `ajax_aff_rename_class_category()` | Rename a class category |
| `atfrfo_delete_class_category` | `ajax_aff_delete_class_category()` | Delete a class category (reassign classes to Uncategorized) |
| `atfrfo_reorder_class_categories` | `ajax_aff_reorder_class_categories()` | Reorder class categories |

These are added to the existing `class-atfrfo-ajax-handler.php`. No new PHP files are needed for the AJAX layer.

---

## 11. `ATFRFO_Data_Store` Extensions
...
Add a classes migration block (analogous to the existing variables migration block) to handle future schema additions without breaking older `.atfrfo.json` files.
...
**The most significant limitation.** Elementor has not published any PHP hooks, filters, or service classes for reading Global Class data in third-party plugins. The developer documentation explicitly says it will arrive "after 4.0 release." This means ATFRFO must use:
...
1. Installing Elementor on the ATFRFO test site (`elementor-v40-test`)
...
Local Classes (per-element, unique) are stored inside the `_elementor_data` JSON of each individual page/post. They are not surfaced by the global-classes REST endpoint. ATFRFO Phase 3 explicitly excludes Local Classes as out of scope.

### 12.5 50-Class Limit

Elementor currently limits Global Classes to 50 per site (community complaint, GitHub discussion #32277). This is an Elementor-side constraint, not ATFRFO's. ATFRFO should document this in the UI but cannot lift the limit.

### 12.6 wp_remote_get() Loopback Issues on Local Environments

On Local by Flywheel (Jim's development environment), `wp_remote_get()` making a loopback HTTP call can fail due to the local server not accepting loopback connections. ATFRFO must:
...
### 12.7 CSS Properties Not Stored in ATFRFO

ATFRFO stores class metadata only — not the actual CSS property values (colors, font-size, etc.) assigned to each class. This is intentional: those values live in Elementor and would require parsing Elementor's internal style schema (a complex, nested, per-breakpoint/state structure). Phase 3 is a management layer, not a CSS editor.

If Jim later wants ATFRFO to display the actual CSS properties of a class, this would be a Phase 3.x extension requiring significant additional research into Elementor's internal style data format.

### 12.8 Responsive Classes Are a Feature Request, Not Yet Implemented

GitHub discussion #34972 ("Responsive CSS Classes") shows that class assignments per breakpoint (applying/removing a class on tablet vs desktop) is an open feature request in Elementor, not yet implemented. ATFRFO's `has_responsive` and `breakpoints` fields track per-class style breakpoints, which IS implemented in Elementor. These are different concepts.
...
- **Create** `includes/class-atfrfo-classes-reader.php` — read-only fetcher + normalizer
- **Modify** `includes/class-atfrfo-data-store.php` — add `classCategories` to map, add Classes CRUD
- **Modify** `includes/class-atfrfo-ajax-handler.php` — add all `atfrfo_sync_classes`, `atfrfo_get_classes`, class meta AJAX endpoints
- **Modify** `includes/class-atfrfo-loader.php` — register `class-atfrfo-classes-reader.php`

Deliverable: `atfrfo_sync_classes` AJAX action returns a normalized class list from Elementor.

### Phase 3.2 — Left Panel

Files to modify:
- **Modify** `admin/js/atfrfo-panel-left.js` — add `renderClassesTree()`, expand `▶ Classes` node
- **Modify** `admin/css/atfrfo-layout.css` — any layout adjustments for class category items (likely minimal; reuses variable category styles)
...
- **Modify** `admin/js/atfrfo-edit-space.js` — add `renderClassesList(category, classes)` handler
- **Create** or extend `admin/css/atfrfo-theme.css` if class-specific color/badge tokens are needed
...
Files to modify:
- **Modify** `admin/js/atfrfo-edit-space.js` or create `admin/js/atfrfo-classes.js` — class detail modal builder
- Modal uses the existing `ATFRFO.Modal.open()` — no new modal infrastructure needed
...
**How to answer:** Inspect the `X-WP-Nonce` header value used by the Elementor editor's own requests. If it matches `wp_create_nonce('wp_rest')`, the standard approach works. If Elementor uses its own nonce action, ATFRFO must replicate it.

### 14.4 Is the 50-class limit enforced client-side only (JS) or server-side (REST validation)?

**Impact on ATFRFO:** If it's only client-side, ATFRFO's read path is unaffected. If server-side, the REST endpoint may only return ≤50 classes regardless.

### 14.5 Are class styles (CSS property values) present in the REST response?

**Partially answered 2026-04-03.** The REST response includes a `variants` array on each class. Classes with no styles defined have `variants: []`. Classes with styles have a non-empty `variants` array. **The internal structure of each variant object is still unknown** — a follow-up capture is needed by expanding one of the non-empty variant arrays in the browser DevTools while the Elementor editor is open. This is **not blocking for Phase 3.1–3.3** (ATFRFO ignores variant data until Phase 3.4). The `has_states` field is conservatively set to `!empty(variants)` until the structure is confirmed.
