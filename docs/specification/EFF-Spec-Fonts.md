# ATFRFO — Fonts Workpage Specification

**Document:** ATFRFO-Spec-Fonts.md
**Version:** 1.0
**Date:** 2026-03-14
**Scope:** The Fonts edit-space — everything rendered inside `#atfrfo-edit-content` when a Fonts subgroup or category is selected in the left nav.
**Related:** ATFRFO-Spec-Colors.md (template), ATFRFO-Spec-Variables.md §2–5

---

## 1. Overview

The Fonts workpage is a full editing environment for CSS custom property font-family variables. It is one of three variable-set workpages (Colors, Fonts, Numbers) that share a common module factory (`ATFRFO.Variables`, `admin/js/atfrfo-variables.js`). The Fonts instance is initialized with `ATFRFO.Variables.initSet(FONTS_CFG)`.
...
var FONTS_CFG = {
    ...
    renderPreviewCell: function (v) {
        return '<span class="atfrfo-font-preview" style="font-family:' + esc(v.value) + '">'
             + 'Aa'
             + '</span>';
    },
...
**Routing:** `initSet()` patches `ATFRFO.EditSpace.loadCategory` to intercept calls where `selection.subgroup === 'Fonts'` and delegate to the shared prototype's `loadVars(selection)`.
...
All Fonts variables have `subgroup === 'Fonts'` and `type === 'font'`. Fields identical to Colors (see ATFRFO-Spec-Colors.md §3.1) except:
...
Identical structure to Colors categories. Stored in `ATFRFO.state.config.fontCategories`.
...
| 3 | 15% | Font preview | `.atfrfo-font-preview` |
| 4 | 1fr | Variable name | `.atfrfo-var-name-input` |
| 5 | 28% | Font value | `.atfrfo-var-value-input` |
| 6 | 12% | Format selector | `.atfrfo-var-format-sel` |
| 7 | 28px | Delete button | `.atfrfo-var-delete-btn` |

No expand button column. No expand panel.

### 5.4 Font Preview Cell (Column 3)

A read-only `<span class="atfrfo-font-preview">` that displays the text **"Aa"** rendered in the variable's font-family value as an inline style.

```html
<span class="atfrfo-font-preview" style="font-family: {v.value}">Aa</span>
```

- Height: 32px (matches the color swatch height in Colors)
- Font size: 18px
- Color: `var(--atfrfo-clr-secondary)`
- If the font is not loaded, the fallback will render; no loading indicator is shown
- Not interactive (no click action)

**Tooltip:** `data-atfrfo-tooltip="Font preview"` (no long form needed)
...
Identical to Colors. `<input type="text" readonly>` — single click removes `readonly`. Enforces `--` prefix on `input` events. Saved via `atfrfo_save_color` AJAX (with `subgroup: 'Fonts'`) on `change` / Enter. Invalid name → revert + field error.
...
- **On save:** AJAX `atfrfo_save_color` with `{id, value, status: 'modified', subgroup: 'Fonts'}`; updates font preview cell and name input in DOM immediately
...
Identical to Colors. `contenteditable` span, single-click to activate, Enter/blur to save, Escape to revert. Saved via `atfrfo_save_category` (with `subgroup: 'Fonts'`).
...
Add button (circle on bottom-left edge of category block) → AJAX `atfrfo_save_color` with defaults from `FONTS_CFG.newVarDefaults`:
...
Delete button (col 7, opacity 0 until row hover) → confirmation modal → AJAX `atfrfo_delete_color` with `{variable_id}`. Fonts variables do not have child variables; the `delete_children` parameter is always `false` and the confirmation modal does not mention children.
...
Identical to Colors. `#atfrfo-fonts-search` input filters rows where `name` or `value` matches the query. Category blocks with all rows hidden are also hidden.
...
Identical to Colors. Font variables are committed to the Elementor kit CSS via `atfrfo_commit_to_elementor`. The commit payload is `{name, value}` pairs for all non-deleted Fonts variables. PHP replaces `--name: value;` in the kit CSS `:root` block or appends new variables.
...
All endpoints require the `atfrfo_admin_nonce` nonce and `manage_options` capability. Fonts variables use the same endpoints as Colors, distinguished by the `subgroup` field in the variable payload.

| Action | POST params | Description |
|--------|-------------|-------------|
| `atfrfo_save_color` | `filename`, `variable` (JSON with `subgroup:'Fonts'`) | Add or update a font variable |
| `atfrfo_delete_color` | `filename`, `variable_id` | Delete a font variable |
| `atfrfo_save_category` | `filename`, `category` (JSON), `subgroup:'Fonts'` | Add or rename a Fonts category |
| `atfrfo_delete_category` | `filename`, `category_id`, `subgroup:'Fonts'` | Delete a Fonts category |
| `atfrfo_reorder_categories` | `filename`, `ordered_ids` (JSON), `subgroup:'Fonts'` | Reorder Fonts categories |
| `atfrfo_commit_to_elementor` | `filename`, `variables` (JSON) | Write Fonts variables to Elementor kit CSS |
...
Fonts uses a parallel class namespace to Colors. Classes are rendered by the shared `atfrfo-variables.js` module with the set name substituted.

| Class | Element | Description |
|-------|---------|-------------|
| `.atfrfo-fonts-view` | Container div | Flex column |
| `.atfrfo-fonts-filter-bar` | Filter bar | Sticky top |
| `.atfrfo-fonts-search` | Search input | Flex-grow |
| `.atfrfo-fonts-add-cat-btn` | Add category button | Icon button |
| `.atfrfo-font-preview` | Font preview cell | Reads `font-family` from inline style; displays "Aa" |
| `.atfrfo-var-name-input` | Name field | Shared class (all sets); monospace, readonly by default |
| `.atfrfo-var-value-input` | Value field | Shared class; renders content in its own `font-family` for Fonts |
| `.atfrfo-var-format-sel` | Format dropdown | Shared class; two options for Fonts |
...
| `.atfrfo-color-list` | Variable rows container | Shared class (name unchanged from Colors) |
| `.atfrfo-color-row` | One variable row | Shared class; 7-col grid for Fonts |
...
Identical to Colors. Reads/writes `ATFRFO.state.variables` (filtered to `subgroup === 'Fonts'`), `ATFRFO.state.config.fontCategories`, `ATFRFO.state.currentFile`. Calls `ATFRFO.App.setDirty()`, `ATFRFO.App.setPendingCommit()`, `ATFRFO.App.refreshCounts()`, `ATFRFO.PanelLeft.refresh()` on the same triggers as Colors.

**`_getVarsForSet()`:**
```javascript
return ATFRFO.state.variables.filter(function (v) { return v.subgroup === 'Fonts'; });
```
